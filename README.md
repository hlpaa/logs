...
-   `event.log`: Server listens to logs
-   `event.query`: Server listens to queries
-   `error.{user_id}`: Server sends alerts to user with id {user_id}
-   `query.{user_id}`: Server sends queries results to user with id {user_id}

## Notas

-   Requisito de negocio para analise de desempenho:
    Analise de picos de erro acontece de maneira sincrona ou assincrona
